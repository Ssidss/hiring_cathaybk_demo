...
請利用 maven 建置一個 Spring-boot 專案,實作以下內容,將結果上傳至
gitHub,並提供連結。
Java:8
...
controller.CoindeskController.newGetCurrentPrice()

### 建立一張幣別與其對應中文名稱的資料表(需附建立 SQL 語法),並提供 查詢 / 新增 / 修改 / 刪除 功能 API。
...
test...api.CurrencyApiTest.getTest()

### 測試呼叫新增幣別對應表資料 API。
test...api.CurrencyApiTest.createTest()

### 測試呼叫更新幣別對應表資料 API,並顯示其內容。
test...api.CurrencyApiTest.updateTest()

### 測試呼叫刪除幣別對應表資料 API。
test...api.CurrencyApiTest.deleteTest()

### 5. 測試呼叫 coindesk API,並顯示其內容。
test...helper.CoindeskHelperTest.getTest()

### 6. 測試呼叫資料轉換的 API,並顯示其內容。
test...api.CoindeskApiTest.getTest